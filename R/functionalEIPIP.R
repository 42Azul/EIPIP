...
#'
#' Set from which train the inside ensembles
#'
#' @param train.set Set from which train the inside ensembles
#' @param val.set Validation set for ensemble selection
#' @param OUTPUT_VAR Variable where our objective is located
#' @param OUTPUT_MIN Value of the minoritary class in the output characteristic
#' @param OUTPUT_MAJ Value of the majoritary class in the output characteristic
#' @param conf Vector of algorithms to be executed calling the metric_optimize metric and the train set
#' @param metrics Metric function which calculates, given a prediction in the form of data[,obs] data[,pred]
#' @param metric_optimize Metric that will be used to train the model
#' @param prop.maj Proportion of the majoritary class
#' @param exhaustive Boolean variable which checks if the training is made in sequential or exhaustive mode
#' @param metric_select Second metric established in order to check if one ensemble is better than the other. They are equal by default.
#' @return List of trained models. Prediction over those models is to be made with the prediction.fold functions.
#' @exports
